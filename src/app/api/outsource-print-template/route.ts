...
export async function GET() {
  ...
  try {
    const row = await prisma.outsourcePrintTemplate.findUnique({
      where: { id: SINGLETON_ID },
    });
    const config = mergeOutsourcePrintConfig(row?.config ?? {});
    return NextResponse.json({
      config,
      updatedAt: row?.updatedAt?.toISOString() ?? null,
    });
  } catch (e) {
    console.error("[GET /api/outsource-print-template]", e);
    return NextResponse.json(
...
export async function PUT(req: Request) {
  ...
  const parsed = parseOutsourcePrintConfigForSave(body.config);
  if (!parsed.ok) {
    return NextResponse.json({ error: parsed.error }, { status: 400 });
  }

  try {
    await prisma.outsourcePrintTemplate.upsert({
      where: { id: SINGLETON_ID },
    ...
    console.error("[PUT /api/outsource-print-template]", e);
    return NextResponse.json(
